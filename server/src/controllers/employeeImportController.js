...
const dateText = (value) => {
    ...
    const text = String(value || "").trim(); return /^\d{4}-\d{2}-\d{2}$/.test(text) ? text : "";
};
...
const previewImport = async (req, res) => {
    try {
        ...
        const [[lookups]] = await Promise.all([db.query("SELECT 1 AS ok")]);
        void lookups;
        ...
        const sourceRows = []; const phones = new Set(); const aadhaars = new Set(); const errors = [];
        for (let rowIndex = 2; rowIndex <= sheet.rowCount; rowIndex += 1) {
            ...
            if (!/^\d{2}:\d{2}$/.test(raw["Duty Start"] || "") || !/^\d{2}:\d{2}$/.test(raw["Duty End"] || "")) rowErrors.push("Duty time must use HH:MM");
            if (!raw["Joining Date"] || (raw.DOB && !raw.DOB)) rowErrors.push("Dates must use YYYY-MM-DD");
        ...
        const uniquePhones = sourceRows.map((item) => item.raw.Phone).filter(Boolean); const uniqueAadhaars = sourceRows.map((item) => item.raw.Aadhaar).filter(Boolean);
        if (uniquePhones.length || uniqueAadhaars.length) { const [existing] = await db.query(`SELECT phone, aadhaar_number AS aadhaarNumber FROM users WHERE phone IN (${uniquePhones.map(() => "?").join(",") || "NULL"}) OR aadhaar_number IN (${uniqueAadhaars.map(() => "?").join(",") || "NULL"})`, [...uniquePhones, ...uniqueAadhaars]); const existingPhones = new Set(existing.map((item) => item.phone)); const existingAadhaar = new Set(existing.map((item) => item.aadhaarNumber)); sourceRows.forEach((item) => { if (existingPhones.has(item.raw.Phone)) item.errors.push("Phone already exists"); if (existingAadhaar.has(item.raw.Aadhaar)) item.errors.push("Aadhaar already exists"); }); }
        const validRows = sourceRows.filter((item) => !item.errors.length); const token = crypto.randomBytes(24).toString("base64url"); previews.set(token, { expiresAt: Date.now() + 15 * 60 * 1000, rows: validRows.map((item) => ({ ...item.raw, branchId: branchMap.get(item.raw["Branch Code"].toUpperCase()), departmentId: departmentMap.get(item.raw["Department Code"].toUpperCase()) })) });
        return res.json({ success: true, previewToken: token, validRows: validRows.map((item) => ({ row: item.rowIndex, name: item.raw.Name, phone: item.raw.Phone, email: item.raw.Email, branchCode: item.raw["Branch Code"], departmentCode: item.raw["Department Code"], aadhaar: mask(item.raw.Aadhaar), pan: mask(item.raw.PAN) })), invalidRows: sourceRows.filter((item) => item.errors.length).map((item) => ({ row: item.rowIndex, name: item.raw.Name, errors: item.errors })), canImport: sourceRows.length > 0 && validRows.length === sourceRows.length });
